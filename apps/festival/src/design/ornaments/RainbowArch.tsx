...
export function RainbowArch({ className = "" }: { className?: string }) {
  ...
  const id = useId();
  const patternId = `arch-checker-${id.replace(/:/g, "")}`;
  return (
    <svg aria-hidden="true" className={`absolute inset-0 h-full w-full ${className}`} viewBox={`0 0 ${w} ${h}`} preserveAspectRatio="none">
      ...
      <g fill="none" strokeLinecap="round">
        ...
        <path d={`M${6 + 5 * step + 1} ${h} V200 A${r0 - 5 * step - 1} ${r0 - 5 * step - 1} 0 0 1 ${w - 6 - 5 * step - 1} 200 V${h}`} stroke={`url(#${patternId})`} strokeWidth={8} />
      </g>
    </svg>
  );
}
