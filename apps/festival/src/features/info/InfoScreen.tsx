...
export function InfoScreen() {
  ...
  return (
    <div className="pt-3">
      ...
      <Card padded={false} className="mt-1.5 px-4 py-1">
        {reminders.supported && (
          <>
            ...
            {reminders.showDenied && <p className="pb-2 text-[13px] text-danger-fg">Notifications are off for this app in Settings. <button type="button" className="underline" onClick={reminders.openSettings}>Open Settings</button></p>}
          </>
        )}
      ...
      {licenses && (
        <Card className="mt-2 text-[14px] leading-5 text-fg-soft">{FONTS.map(([f, l]) => <div key={f}><b className="text-fg">{f}</b> — {l}</div>)}<div className="mt-1">Poster artwork © SBG Productions, used with permission.</div><div className="mt-1">Festival photography © Alive Coverage, used with permission of SBG Productions.</div><div className="mt-1">Sierra Nevada® is a registered trademark of Sierra Nevada Brewing Co.</div></Card>
      )}
