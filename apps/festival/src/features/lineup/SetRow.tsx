...
export function SetRow({ set, artist, stage, now, showStage = false, dayLabel, tag }: {
  ...
  // Where a set is held always wins over the artist's tier label; no venue line when it just repeats the section.
  const hasPlace = showStage || (set.venue && set.venue !== stage.name);
  const sub = live ? `On now · ${minutesLeft(set, now)} min left` : ended ? "Ended" : hasPlace ? placeOf(set, stage) : artist.tier === "musicmaker" ? "Music Maker Foundation" : isHeadliner ? "Headliner" : undefined;
  return (
    <div className={`-mx-4 flex items-center gap-3 border-b border-hair px-4 py-2.5 ${live || isHeadliner ? "bg-gradient-to-r from-sun/20 to-transparent" : ""} ${ended ? "opacity-85" : ""}`}>
      <button type="button" onClick={() => navigate(`/lineup/artist/${artist.id}`)} className="flex min-w-0 flex-1 items-center gap-3 text-left">
        <span className={`w-20 shrink-0 text-[14px] font-semibold leading-[18px] tabular-nums ${live ? "text-fg" : "text-fg-soft"}`}>{dayLabel ? `${dayLabel} ` : ""}{live ? "● " : ""}{formatTime(start)}{set.stageId !== "juke" && <span className="block text-[12px] font-normal leading-4 text-fg-soft">– {formatTime(parseIso(set.end))}</span>}</span>
