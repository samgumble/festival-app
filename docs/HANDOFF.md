...
| Plan / Alerts / Info / PWA | ✅ Sep 10 | Plan/Alerts/Info at rough fidelity; PWA: Workbox precache (2605 KiB (grew by the Capacitor web-fallback chunks)), prompt-mode update banner, Info "Get the app" nudge (Chromium prompt / iOS sheet), offline e2e `npm run e2e:a11y                        # axe-core WCAG gate, every screen × both themes (needs network for axe; docs/COMPLIANCE.md §5)
npm run e2e:offline`. GitHub Pages serves `sw.js` with `Cache-Control: max-age=600`, so a new build can take up to ~10 min to surface the update banner |
