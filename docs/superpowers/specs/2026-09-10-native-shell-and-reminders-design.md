...
| `share.ts` | `shareText(title, text): Promise<void>`, `shareFile(filename, mime, text): Promise<void>` | `Share.share({ title, text })`; `shareFile` shares the file body as text with the filename in the title (v1 limitation: `@capacitor/filesystem` is not on the approved list) | `navigator.share`/clipboard for text; existing `downloadText` for files |
| `notifications.ts` | `isSupported(): boolean`, `permission(): Promise<"granted" \| "denied" \| "prompt">`, `request(): Promise<"granted" \| "denied">`, `ensureExact(): Promise<boolean>` (Android 12+ exact-alarm setting; `true` elsewhere), `pending(): Promise<Array<{ id: number; at?: number }>>`, `schedule(items: ReminderItem[]): Promise<void>`, `cancel(ids: number[]): Promise<void>`, `onTap(handler: () => void): () => void` | `LocalNotifications.*` | `isSupported() === false`; everything else resolves without effect |
...
- Simulator smoke (controller, via the iOS Simulator tool): app launches to Now, favorite → haptic call logged, Plan settings toggle asks permission, and after favoriting a future set the DEV-only `?pending` readout on Plan settings (see plan) lists one pending notification at `start − lead`. A real firing is verified on Sam's iPhone with a set that starts within the lead time, or by the simulator with the lead set to 30 min and the device clock left real (festival sets are in the future, so pending entries are the verifiable artifact).
